...
public class TanksGame extends Applet implements Runnable, KeyListener, MouseListener, MouseMotionListener {
    ...
    @Override
    //Chosing which window is to be updated and painted
    public void paint(Graphics g) {
        switch(currentWindow) {
            ...
            case 4: { //Multiplayer menu window
                if(currentWindowChanged) {
                    multiplayerMenu = new MultiplayerMenuWindow(IP, port);
                    currentWindowChanged = false;
                }
                ...
                switch(multiplayerMenu.getButtonClicked()) {
                    ...
                    case 1:
                        ...
                        currentWindowChanged = true;
                        levelSelected = 100;
                        IP = multiplayerMenu.getIP();
                        port = Integer.parseInt(multiplayerMenu.getPort());
            ...
            case 11: { //Level creation window
                if(currentWindowChanged) {
                    currentWindowChanged = false;
                    levelWindow = new LevelWindow(levelSelected, equipment, character, enemyTank, IP, port);
                    loadLevelPictures();                    
                }
                
                if(escKey) {
                    if(levelWindow.isMultiplayer() && !levelWindow.isConnected()) {
                        currentWindow = 4;
                        currentWindowChanged = true;
                    }
                    else if(levelWindow.isLevelEnd() && levelWindow.isMultiplayer()) {
                        currentWindow = 0;
                        currentWindowChanged = true;
                    }
                    else if(levelWindow.isLevelEnd() && !levelWindow.isMultiplayer()) {
                        ...
                        if(levelWindow.isLevelCompleted()) {
                            if(levelWindow.getCurrentLevel() >= equipment.getLevelCompleted())
                                equipment.setLevelCompleted(levelWindow.getCurrentLevel());
                        }
                ...
                switch(levelWindow.getButtonClicked()) {
                    ...
                    case 1:
                        ...
                        currentWindowChanged = true;
                        try {
                            levelWindow.getSocket().close();
                        }
                        catch(IOException ex) { 
                        }
                        
                        try {
                            levelWindow.getServerSocket().close();
                        }
                        catch(IOException ex) { 
                        }
                        break;
    ...
    //Sending level pictures to levelWindow when its created
