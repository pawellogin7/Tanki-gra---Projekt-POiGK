...
public class Player {
	...
	private int speedY = 0;
        ...
        private Point corner3 = new Point(0, 0);
        private Point corner4 = new Point(0, 0);
        ...
        //Player class is responsible for handling our tank
        Player(Equipment eq, int tankSlot, int positionX, int positionY) {
            //setting current Player values depending on current tank slot equipment
            posX = positionX;
            posY = positionY;
            bodyRotationAngle = 0;
            ...
            cornerAngle = Math.atan2(32, 64);
            cornerRadius = (int) Math.sqrt(Math.pow(32, 2) + Math.pow(64, 2));
            updateCornerValues();
	...
	public void update(int mx, int my) {
        ...
        private void updateCornerValues() {
            corner1.x = posX + (int) Math.round(cornerRadius * Math.cos(bodyRotationAngle + cornerAngle) );
            corner1.y = posY + (int) Math.round(cornerRadius * Math.sin(bodyRotationAngle + cornerAngle) );
            
            corner2.x = posX + (int) Math.round(cornerRadius * Math.cos(bodyRotationAngle + Math.toRadians(180) + cornerAngle) );
            corner2.y = posY + (int) Math.round(cornerRadius * Math.sin(bodyRotationAngle + Math.toRadians(180) + cornerAngle) );
            
            corner3.x = posX + (int) Math.round(cornerRadius * Math.cos(bodyRotationAngle + Math.toRadians(180) - cornerAngle) );
            corner3.y = posY + (int) Math.round(cornerRadius * Math.sin(bodyRotationAngle + Math.toRadians(180) - cornerAngle) );
            
            corner4.x = posX + (int) Math.round(cornerRadius * Math.cos(bodyRotationAngle - cornerAngle) );
            corner4.y = posY + (int) Math.round(cornerRadius * Math.sin(bodyRotationAngle - cornerAngle) );
        }
        
        
        //Detects collision with tiles
        public void detectCollision(int tileLeftValue, int tileRightValue, int tileUpValue, int tileDownValue) {
            int tileLeft = (int) Math.floor(posX / 250) * 250;
            int tileRight = (int) Math.floor(posX / 250) * 250 + 250;
            int tileUp = (int) Math.floor(posY / 250) * 250 - 7;
            int tileDown = (int) Math.floor(posY / 250) * 250 + 250 + 7;
            if(movingLeft && tileLeftValue != 0) {
                if(corner1.x < tileLeft) {
                    movingLeft = false;
                    int pos1 = posX;
                    posX += tileLeft - corner1.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
                else if(corner2.x < tileLeft) {
                    movingLeft = false;
                    int pos1 = posX;
                    posX += tileLeft - corner2.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
                else if(corner3.x < tileLeft) {
                    movingLeft = false;
                    int pos1 = posX;
                    posX += tileLeft - corner3.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
                else if(corner4.x < tileLeft) {
                    movingLeft = false;
                    int pos1 = posX;
                    posX += tileLeft - corner4.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
            }
            else if(movingRight && tileRightValue != 0) {
                if(corner1.x > tileRight) {
                    movingRight = false;
                    int pos1 = posX;
                    posX += tileRight - corner1.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
                else if(corner2.x > tileRight) {
                    movingRight = false;
                    int pos1 = posX;
                    posX += tileRight - corner2.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
                else if(corner3.x > tileRight) {
                    movingRight = false;
                    int pos1 = posX;
                    posX += tileRight - corner3.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
                else if(corner4.x > tileRight) {
                    movingRight = false;
                    int pos1 = posX;
                    posX += tileRight - corner4.x;
                    speedX -= posX - pos1;
                    updateCornerValues();
                }
            }
            
            if(movingUp && tileUpValue != 0) {
                if(corner1.y < tileUp) {
                    movingUp = false;
                    int pos1 = posY;
                    posY += tileUp - corner1.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
                else if(corner2.y < tileUp) {
                    movingUp = false;
                    int pos1 = posY;
                    posY += tileUp - corner2.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
                else if(corner3.y < tileUp) {
                    movingUp = false;
                    int pos1 = posY;
                    posY += tileUp - corner3.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
                else if(corner4.y < tileUp) {
                    movingUp = false;
                    int pos1 = posY;
                    posY += tileUp - corner4.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
            }
            else if(movingDown && tileDownValue != 0) {
                if(corner1.y > tileDown) {
                    movingDown = false;
                    int pos1 = posY;
                    posY += tileDown - corner1.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
                if(corner2.y > tileDown) {
                    movingDown = false;
                    int pos1 = posY;
                    posY += tileDown - corner2.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
                if(corner3.y > tileDown) {
                    movingDown = false;
                    int pos1 = posY;
                    posY += tileDown - corner3.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
                if(corner4.y > tileDown) {
                    movingDown = false;
                    int pos1 = posY;
                    posY += tileDown - corner4.y;
                    speedY -= pos1 - posY;
                    updateCornerValues();
                }
            }
        ...
        //Change primary weapon when you click 1, 2 or 3
        ...
        //Shooting primary weapon on mouse left
        ...
        //Shooting secondary weapon on mouse right
        ...
        //using ability, when space was clicked
        ...
        //Deals damage on collision with pojectile
        ...
        //checks if collision with projectile appeared
